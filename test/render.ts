...
const p = (content: ABI.VariantInlineNodes[]) => ['paragraph', {content}] as ABI.VariantBlockNodes
...
describe('inline renderer', function() {
    ...
    it('should prioritize code marks', function() {
        const marks: ABI.VariantMarkNodes[] = [['bold', {}], ['code', {}]]
        assert.equal(renderInline([text('foo', marks)]), '<code>foo</code>')
    })
    it('should add style marks in correct order', function() {
        const marks1: ABI.VariantMarkNodes[] = [['bold', {}], ['strike', {}], ['italic', {}]]
        const marks2: ABI.VariantMarkNodes[] = [['italic', {}], ['strike', {}], ['bold', {}]]
    ...
    it('should not add marks to empty text', function() {
        const marks: ABI.VariantMarkNodes[] = [['strike', {}], ['italic', {}]]
        assert.equal(renderInline([text('', marks)]), '')
    })
    it('should throw on unknown mark', function() {
        assert.throws(() => {
            const marks: any[] = [['bold', {}], ['foo', {}]]
            renderInline([text('foo', marks)])
...
describe('block renderer', function() {
    ...
    describe('paragraph', function() {
        ...
        it('should render inline', function() {
            assert.equal(
                renderBlock(
                    p([text('foo'), ['hard_break', {}], text('bar '), text('baz', [['bold', {}]])])
                ),
    ...
    describe('heading', function() {
        ...
        it('should only render title when first or below first image', function() {
            assert.equal(
                renderBlock(['heading', {value: 'foo', level: 1}], {numSections: 1}),
                '<h2>foo</h2>'
            )
            assert.equal(
                renderBlock(['heading', {value: 'foo', level: 1}], {numImages: 1}),
                '<h1>foo</h1>'
            )
            assert.equal(
                renderBlock(['heading', {value: 'foo', level: 1}], {numImages: 2}),
                '<h2>foo</h2>'
            )
        })
    })
    ...
    describe('image', function() {
        it('should render', function() {
            assert.equal(
                renderBlock([
                    'image',
                    {src: 'https://example.com/image.jpg', caption: [], layout: 1},
                ]),
    ...
    describe('divider', function() {
        it('should render', function() {
            assert.equal(renderBlock(['divider', {}]), '<hr>')
        })
    })
        ...
        it('should render bullet', function() {
            assert.equal(
                renderBlock([
                    ...
                    {
                        type: 0,
                        items: [{content: [text('foo')]}, {content: [text('bar')]}],
                    },
                ]),
    ...
    before(function() {
        ...
        for (let i = 0; i < jsonFiles.length; i++) {
            const doc = JSON.parse(
                fs.readFileSync('./test/documents/' + jsonFiles[i]).toString('utf8')
            )
